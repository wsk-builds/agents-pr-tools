...
Area detection is heuristic. Explicit `docs...` and `test...` titles take priority, then the tool looks at normalized labels, including package-scoped labels such as `package:agents-core`, followed by other PR title prefixes and keywords. Unknown work is grouped as `other`.
